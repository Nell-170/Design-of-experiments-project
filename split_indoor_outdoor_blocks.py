...
# Input/output function that receives user input via console 
# and returns the parsed arguments for the specified files
def parse_args():
    ...
    # If this flag is included, images without keypoints will be accepted
    parser.add_argument("--include-non-keypoints", action="store_true")
    return parser.parse_args()

# Test execution: python split_indoor_outdoor_blocks.py
def main():
    ...
    output_dir.mkdir(parents=True, exist_ok=True) # Creates the directory if it does not exist (output-dir)
    ...
    # Category name-to-ID mapping dictionary
    ...
    # Maps image IDs to their file names (.jpg)
    image_file_by_id = {img["id"]: img["file_name"] for img in instances["images"]}

    image_categories = defaultdict(set) # Special dictionary initialization
    
    # Creates a dictionary entry for each image with its corresponding categories
